...
// MiniStack emulates ElastiCache: CreateCacheCluster spins up a real redis process
...
//
// hostOverride: MiniStack publishes each cluster's port to the HOST, but Lambda runs
// INSIDE the ministack container where `localhost` = the container, not the host.
...
// ponytail: Linux needs ministack container launched with --add-host=host.docker.internal:host-gateway.
