...
export function lintApp(cfg: AppConfig, cwd: string): void {
  ...
  for (const [name, fn] of Object.entries(cfg.functions ?? {})) {
    ...
    if (dot < 1) {
      errors.push(
        `function ${name}: handler '${fn.handler}' must be '<file>.<export>' (e.g. ./src/api.handler)`,
      );
      continue;
    }
    ...
    if (!existsSync(entry)) {
      errors.push(
        `function ${name}: handler file not found: ${filePart}.ts (handler: ${fn.handler})`,
      );
      continue;
    }
...
function exportsSymbol(src: string, name: string): boolean {
  ...
  if (new RegExp(`export\\s+(async\\s+)?(function|const|let|var|class)\\s+${n}\\b`).test(src))
    return true;
