...
One ElastiCache cluster per `caches.<name>`, provisioned via `CreateCacheCluster` against Floci. Floci assigns sequential ports starting at 16379 and publishes them to the host. Lambda (executing inside the floci container) cannot reach `localhost:16379` — the CLI substitutes `host.docker.internal:16379` in the env var so Lambda hits the host-published port. The UI inspector (on the host) uses the raw `localhost` endpoint from `DescribeCacheClusters`.

### Databases locally

Four database types, each driven through its native API against Floci:
...
buckets: { name: {} }
databases: { name: { type: dynamodb|postgres|mysql|external, ... } } # dynamodb: partitionKey, sortKey?, gsi? — postgres/mysql: instanceClass?, storage?, multiAz?, name?, init_sql?. dynamodb/postgres/mysql provisioned via their APIs; external = BYO URL
...
| `packages/ui/server/inspect.ts` → `getBucket` | Fetches all S3 control-plane properties; each call safe-wrapped — Floci may return errors for unsupported APIs; UI renders AWS default label ("Disabled", "0 configurations") when undefined |
